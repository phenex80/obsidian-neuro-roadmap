...
/** Debounces index changes and serializes reconciliations to avoid Graph request storms. */
    ...
    const run = this.queue.then(async () => {
      ...
      try {
        ...
        if (!this.disposed) {
          ...
          this.setStatus({
            phase: 'error',
            message: error instanceof Error ? error.message : 'Microsoft calendar sync failed.',
          });
        }
        throw error;
      }
