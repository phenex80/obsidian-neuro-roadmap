...
Google Calendar sync is optional and desktop-only. Neuro Roadmap does not operate a backend service that receives your Google OAuth credentials, access tokens, refresh tokens, or Calendar data. When you explicitly configure and connect Google Calendar, communication takes place directly between the plugin running in Obsidian and Google's services. The plugin sends only the calendar projection needed to create, update, verify, or delete managed Google events described in the [Google Calendar setup guide](google-calendar-setup.md): title, scheduled date/range, description with source context, reminder policy, and the stable item identity needed to manage the event. The configured OAuth client values remain in local plugin settings, the refresh token is held in Obsidian SecretStorage, and access tokens and OAuth codes stay in memory. The plugin does not import Google Calendar edits into Markdown.
