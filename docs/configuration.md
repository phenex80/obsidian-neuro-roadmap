...
- Due date: `due_date, due, deadline`
...
Use **Excluded folders / paths** for hard exclusions. **Roadmap source scope** can index every Markdown file (the compatible default) or only notes matching configured property/value rules. The template guard ignores mapped values that truly identify templates; `roadmap` remains a valid roadmap-note value.
